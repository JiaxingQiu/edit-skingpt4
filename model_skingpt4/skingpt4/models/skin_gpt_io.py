...
@registry.register_model("skin_gpt_io")
class skingpt_io(Blip2Base):
    """
    ...
    def forward(self, samples):
        ...
        if ("prompt_input" in samples) and ("answer_output" in samples):
            ...
            targets_tail = to_regress_tokens.input_ids.masked_fill(
            ...
            to_regress_attn = to_regress_tokens.attention_mask
            id_dtype = to_regress_tokens.input_ids.dtype
            id_device = to_regress_tokens.input_ids.device

        # Prepend BOS and image tokens; mask them out in labels
        empty_targets = (
            torch.ones([atts_img.shape[0], atts_img.shape[1]+1],
                       dtype=torch.long).to(image.device).fill_(-100)  # plus one for bos
        )
        targets = torch.cat([empty_targets, targets_tail], dim=1)

        batch_size = img_embeds.shape[0]
        bos = torch.ones([batch_size, 1], dtype=id_dtype, device=id_device) * self.llm_tokenizer.bos_token_id
        bos_embeds = self.llm_model.model.embed_tokens(bos)
        atts_bos = atts_img[:, :1]

        inputs_embeds = torch.cat([bos_embeds, img_embeds, to_regress_embeds], dim=1)
        attention_mask = torch.cat([atts_bos, atts_img, to_regress_attn], dim=1)
    ...
    def run_epoch(self, loader, optimizer=None, scaler=None):
        ...
        for batch in loader:
            # Move only tensors to device; keep other fields intact
            samples = dict(batch)
